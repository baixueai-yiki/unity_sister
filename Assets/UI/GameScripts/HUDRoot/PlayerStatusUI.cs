...
{
    public CharacterAttributes characterAttributes;               //声明一个变量，命名为stats，指向场景中的PlayerStats
    ...
    {   //这些都是让组件在运行游戏时自动绑定的，可以让这些组件没必要在编辑器里手动选定
        ...
        characterAttributes = GameObject.Find("Player").GetComponent<CharacterAttributes>();      // 获取自身对象的 PlayerController 组件并赋值给 controller
    }


    void Update()    //每一帧都调用的函数
    {
        if (characterAttributes == null) return;   // 若 变量stats没有被赋值（即没有找到PlayerStats组件）则 直接返回


        Bar_Stamina.maxValue = characterAttributes.maxStamina;   // 同步最大值
        Bar_Stamina.value = characterAttributes.stamina;         // 直接用当前体力
